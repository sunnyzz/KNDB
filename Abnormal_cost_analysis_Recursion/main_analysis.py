...
if __name__ == "__main__":
	...
	def finance_analysis(material,version_1,version_2,use_1,use_2,lost_1,lost_2):
		...
		print('+===========================================================+')				
		class_cost,cost_diff_ = cost_mining.check_unitcost_menge_ausch(material,version_1,version_2,use_1,use_2,lost_1,lost_2)
		
		if class_cost:
			print('+___________________________________________________________+')
			#cost_mining.cost_stage_component_check(material,version_1,version_2)
			
			cost_stage,_diff_value = cost_mining.cost_stage_component_check(material, version_1, version_2,cost_diff_)	#本材、本阶总成本（不含本材）阈值判别
			
			if cost_stage:	  #本阶总成本（不含本材）阈值判别为1
				process_add, process_same, process_red = bsa.process_bom_check(material, version_1, version_2)
				
				for process_name in process_add:	#增加的工序逐个阈值判别
					
					cost_mining.process_add_check(material, process_name, version_1, version_2)
				
				for process_name in process_red:	#减少的工序成本记录
					
					cost_mining.process_red_record(material, process_name, version_1, version_2)
			...
			cost_accumulated_value,diff_value_ = cost_mining.cost_accumulated_component_diff(material,version_1,version_2,cost_diff_)
		
			if cost_accumulated_value:
				print('+___________________________________________________________+')
				bom_add,bom_same,bom_red =	bsa.check(material,version_1,version_2)
			
				for i_code in bom_add:
				
					cost_mining.sub_cost_total_diff_with_matnr(material,version_1,i_code,version_2)
					
				for i_code in bom_red:
					cost_mining.sub_bom_red_record(material,version_1,i_code,version_2)
